...
function sumOfCodes(str) {
  return typeof str !== 'string' || str.length === 0
    ? 0
    : str.split('').reduce((acc, val) => acc + val.charCodeAt(), 0);
}
...
function isPalindrome(str) {
  ...
  for (let i = 1; i < input.length - 1; i += 1) {
    if (input.slice(i - 1, i) !== input.at(-i)) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  const wordsLength = sentence.split(' ').map((word) => word.length);
  const sortedLength = wordsLength.sort((a, b) => b - a);
  return sentence
    .split(' ')
    .filter((word) => word.length === sortedLength[0])[0];
}
...
function encodeToRot13(str) {
  ...
  const result = arrOfChars.map((char) => {
    if (abc.indexOf(char) === -1) return char;
    let rotNIndex = abc.indexOf(char) + rotN;
    if (rotNIndex > abc.length - 1) {
      rotNIndex -= abc.length;
    }

    if (char === char.toUpperCase()) return abc[rotNIndex].toUpperCase();
    return abc[rotNIndex].toLowerCase();
  });
  return result.join('');
}
