...
function sumOfCodes(str) {
  if (typeof str !== 'string' || str.length < 1) return 0;
  const [first, ...rest] = str;
  return sumOfCodes(rest.join('')) + first.charCodeAt(0);
}
...
function orderAlphabetically(str) {
  if (str.length < 1) return '';
  const [first, ...rest] = str;
  const result = orderAlphabetically(rest) + first;
...
function isPalindrome(str) {
  ...
  const temp = str.toLowerCase().replace(regex, '');
  for (let i = 0; i < temp.length / 2; i += 1) {
    if (temp[i].toLowerCase() !== temp[temp.length - i - 1].toLowerCase())
      return false;
  }
  return true;
}
...
function invertCase(str) {
  return [...str]
    .map((val) =>
      val !== val.toLowerCase() ? val.toLowerCase() : val.toUpperCase()
    )
    .join('');
}
...
function extractNameFromTemplate(value) {
  return removeLastOccurrences(removeFirstOccurrences(value, 'Hello, '), '!');
}
...
function encodeToRot13(str) {
  return [...str]
    .map((val) => {
      const isNotCapital = val !== val.toUpperCase();
      let newVal = val;

      if (isNotCapital) newVal = val.toUpperCase();
      if (newVal.charCodeAt() >= 65 && newVal.charCodeAt() <= 90) {
        newVal = String.fromCharCode(
          ((newVal.charCodeAt() - 65 + 13) % 26) + 65
        );
      }
      if (isNotCapital) newVal = newVal.toLowerCase();
